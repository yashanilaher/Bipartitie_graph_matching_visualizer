...
export default function GaleVisualizer({
  ...
  // current snapshot
  const { manStatus, womanStatus, engaged, isMenProposer, freeList } =
    steps[idx];
        ...
        <div className="visualizer">
            ...
            <div className="status-panel">
                ...
                <div className="engagements">
                    ...
                    {Object.entries(engaged).map(([q, p]) => (
                        <li key={q}>
                        <span className="person-name">{renderSubscript(p)}</span>{' '}
                        ↔{' '}
                        <span className="person-name">{renderSubscript(q)}</span>
                        </li>
                    ))}
                    </ul>
                </div>
            ...
            <div className="participants-grid">
                ...
                <h2>Men</h2>
                {Object.keys(menPreferences).map((m) => {
                    const isEngaged = isMenProposer
                    ? Object.values(engaged).includes(m)
                    : Object.keys(engaged).includes(m);
                    const partner = Object.keys(engaged).find(
                    (w) => engaged[w] === m
                    );
                    return (
                    <div key={m} className="person-row">
                        <div
                        className={`avatar ${
                            isEngaged ? 'engaged highlight-circle' : ''
                        }`}
                        >
                        <span className="person-name">
                            {renderSubscript(m)}
                ...
                {Object.keys(womenPreferences).map((w) => {
                    const isEngaged = isMenProposer
                    ? Object.keys(engaged).includes(w)
                    : Object.values(engaged).includes(w);
                    const partner = engaged[w];
                    return (
                    <div key={w} className="person-row">
                        <div
            ...
            <div className="legend">
                ...
                <div className="legend-item">
                    <span className="dot accepted matched"></span>{' '}
                    Accepted & Matched
                </div>
                <div className="legend-item">
