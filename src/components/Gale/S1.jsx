...
//   // initialize
//   men.forEach(m => manStatus[m] = menPreferences[m].map(() => 0));
//   women.forEach(w => womanStatus[w] = womenPreferences[w].map(() => 0));
...
//       // assume rejection
//       manStatus[p][i] = 2;
//       const rankP = proposeePrefs[q].indexOf(p);
//       if (isMenProposer) womanStatus[q][rankP] = 2;
//       else manStatus[q][rankP] = 2;
...
//         if (proposeePrefs[q].indexOf(p) < proposeePrefs[q].indexOf(current)) {
//           // swap
//           engaged[q] = p;
//           manStatus[p][i] = 1;
//           if (isMenProposer) womanStatus[q][rankP] = 1;
...
export default function S1() {
  ...
  // useEffect(() => {
  //   setSteps(stableMatchingWithSteps(menPrefs, womenPrefs, 'men'));
  // }, []);
